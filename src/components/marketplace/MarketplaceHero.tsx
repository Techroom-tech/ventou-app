...
export default function MarketplaceHero() {
  ...
  if (isLoading) {
    return <Skeleton className="w-full aspect-[21/9] md:aspect-[3/1] rounded-2xl" />;
  }

  if (!banners?.length) {
    return (
      <div className="w-full rounded-2xl bg-gradient-to-br from-primary via-primary/90 to-accent/80 flex items-center p-8 md:p-14 relative overflow-hidden">
        {/* Decorative shapes */}
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="text-primary-foreground/80 text-sm md:text-lg mb-6 max-w-lg">
            Des milliers de produits de vendeurs vérifiés à travers l'Afrique. Livraison rapide et paiement sécurisé.
          </p>
  ...
  return (
    <div className="relative group rounded-2xl overflow-hidden shadow-lg">
      ...
      {banners.length > 1 && (
        <>
          <button onClick={scrollPrev} className="absolute left-3 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full bg-background/90 backdrop-blur flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg hover:bg-background">
            <ChevronLeft className="h-5 w-5" />
          </button>
          <button onClick={scrollNext} className="absolute right-3 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full bg-background/90 backdrop-blur flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg hover:bg-background">
